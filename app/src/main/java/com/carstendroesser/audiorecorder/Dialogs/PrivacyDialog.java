...
public class PrivacyDialog extends AlertDialog {
    ...
    public PrivacyDialog(Context pContext) {
        super(pContext);

        getWindow().getAttributes().windowAnimations = R.style.DialogsAnimation;

        // inflate view
        View content = LayoutInflater.from(pContext).inflate(R.layout.dialog_privacypolicy, null);
        setView(content);
    ...
    private void load() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    InputStream stream = getContext().getAssets().open("privacypolicy.txt");
